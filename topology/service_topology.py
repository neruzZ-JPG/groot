...
def get_service_topology(dir_name):
    G = nx.DiGraph()
# 遍历当前目录下的所有文件
...
def draw(G: nx.DiGraph):
    ...
    plt.savefig("service_topology.png")

if __name__ == '__main__':
    G = get_service_topology("../trace_data")
    draw(G)
    print(G.nodes)
    print("G has " + str(len(G.nodes)) + " nodes")
    nx.write_gml(G, "service_topology.gml")